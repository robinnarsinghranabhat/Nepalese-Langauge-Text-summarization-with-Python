...
def _format_results(extracted_sentences, split,seperator = False):
...
def summarize_nepali(text, ratio=0.2, word_count=None, split=False,seperator = False):
    """Get a summarized version of the given nepalese text.
    ...
    seperator : bool , optional
	if True, the nepalese sentences will be joined by unicode character "।" that's just as "fullstop" for english
    Returns
    -------
    list of str
        If `split` **OR**
    ...
    if len(sentences) == 0:
        ...
        return('Input seems empty')
#         return [] if split else u""

    # If only one sentence is present, the function raises an error (Avoids ZeroDivisionError).
    if len(sentences) == 1:
#         raise ValueError("input must have more than one sentence")
        return('Short news , no summary needed')
    ...
    if not most_important_docs:
        ...
        return "Couldn't get revelant sentences"
#         return [] if split else u""
    ...
    return _format_results(extracted_sentences, split , seperator).replace('\r\n',"")
